...
class DeepSeekClient:
    ...
    def batch_generate(self, prompts: list, max_tokens: int = 200, 
                      temperature: float = 0.7, batch_size: int = 5, 
...
class MockDeepSeekClient:
    ...
    def __init__(self, api_key: str = "mock_key"):
        ...
        self.question_templates = [
            "如何优化{alloy}的{property}性能，同时保持其{constraint}特性？",
            "在{application}应用中，如何通过调整{alloy}的{parameter}来改善{target}？",
            "如果要在{condition}下使用{alloy}，需要进行哪些成分或工艺调整？",
            "如何设计一种替代{alloy}的新材料，要求{requirements}？",
            "在{alloy}的基础上，如何平衡{conflict1}和{conflict2}的性能要求？"
        ]
    ...
    def generate_text(self, prompt: str, max_tokens: int = 200, 
        ...
        # 从提示中提取信息
        alloy_info = self._extract_alloy_info(prompt)
        ...
        question = template.format(
            alloy=alloy_info.get('alloy', '钛合金'),
            property=alloy_info.get('property', '力学性能'),
            constraint=alloy_info.get('constraint', '加工性能'),
            application=alloy_info.get('application', '航空航天'),
            parameter=alloy_info.get('parameter', '热处理工艺'),
            target=alloy_info.get('target', '疲劳寿命'),
            condition=alloy_info.get('condition', '高温高压'),
            requirements=alloy_info.get('requirements', '成本降低20%'),
            conflict1=alloy_info.get('conflict1', '强度'),
            conflict2=alloy_info.get('conflict2', '韧性')
        )
        
        return question
    
    def _extract_alloy_info(self, prompt: str) -> Dict[str, str]:
        """从提示中提取合金信息"""
        info = {}
...
if __name__ == "__main__":
    ...
    test_prompt = """
基于TC4钛合金数据生成一个反事实设计问题。
TC4具有优良的强度和韧性，应用于航空航天领域。
"""
