...
from config import SCORE_MODEL, SCORE_ROUND
...
class ScoreProcessor:
    ...
    def __init__(self):
        ...
        self.output_dir = '/path/to/AlloyDatasetBuilding/AnswerScore2'
    ...
    def get_intermediate_file(self, question_type: str) -> str:
        ...
        return original_file + f".intermediate_{SCORE_ROUND}"
    ...
    def process_single_question(self, question: Dict[str, Any], question_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            question = self.score_generator.add_scores_to_question(question, score_result, question_type)
    ...
    def run_all(self, question_types: list = None, 
        ...
        print(f"\n{'='*60}")
        print(f"所有问题类型处理完成（第二轮评分：{SCORE_ROUND}）")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    print(f"\n评分模型: {SCORE_MODEL}")
    print(f"评分轮次: {SCORE_ROUND}")
    print(f"支持的问题类型: {processor.question_types}")
    
    import argparse
    parser = argparse.ArgumentParser(description='第二轮批量评分')
