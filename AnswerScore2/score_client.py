...
# -*- coding: utf-8 -*-
"""
第二轮评分大模型客户端 - 支持Kimi、DeepSeek、Qwen、GLM和MiniMax
...
from config import DEEPSEEK_CONFIG, QWEN_CONFIG, GLM_CONFIG, MINIMAX_CONFIG, KIMI_CONFIG, SCORE_MODEL
...
class ScoreClient:
    ...
    def __init__(self):
        if SCORE_MODEL.lower() == 'kimi':
            config = KIMI_CONFIG
            self.model_name = config["model"]
        elif SCORE_MODEL.lower() == 'deepseek':
            ...
            print(f"警告：未知的SCORE_MODEL '{SCORE_MODEL}'，默认使用Kimi")
            config = KIMI_CONFIG
            self.model_name = config["model"]
        ...
        print(f"第二轮评分大模型客户端初始化完成，使用模型: {self.model_name}")
